...
Time::DATE_FORMATS[:default] = lambda {|date| date.strftime("%-m/%-d/%Y %l:%M%P").strip} # 1/9/2012 1:03pm
Time::DATE_FORMATS[:long] = "%B %e, %Y %l:%M%P" # January 09, 2012 1:30pm

# Date
# See http://api.rubyonrails.org/classes/Date.html

Date::DATE_FORMATS[:default] = lambda {|date| date.strftime("%-m/%-d/%Y").strip} # 1/9/2012
