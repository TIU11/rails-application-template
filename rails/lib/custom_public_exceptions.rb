class CustomPublicExceptions < ActionDispatch::PublicExceptions
  ...
  def call(env)
    begin
      ...
      %@app_name.camelize%::Application.routes.recognize_path("/#{@status_code}")
      %@app_name.camelize%::Application.routes.call env # Look for a route to a custom error handler
